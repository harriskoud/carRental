...
public class CarResource {
    ...
    public
    @ResponseBody
    ResponseEntity<?> getCar(@PathVariable(name = "brandName") String brandname) {
    ...
    public
    @ResponseBody
    ResponseEntity<?> createCar(@RequestBody Car car) {
    ...
    public
    @ResponseBody
    ResponseEntity<?> updateBookedDateGet(@PathVariable(name = "brandName") String brandname,
    ...
    public
    @ResponseBody
    ResponseEntity<?> updateBookedDatePut(@RequestBody Car car) {
    ...
    public
    @ResponseBody
    ResponseEntity<?> getCarFromUser(@PathVariable(name = "brandName") String brandname) {
...
/*	@GetMapping("/elastic/{brandName}")
    public @ResponseBody ResponseEntity<?> getCarWithElastic(@PathVariable(name = "brandName") String brandname) {

		log.info("Get Car Info");
		Car car = carElasticRepository.findByBrandName(brandname);
		return ResponseEntity.ok(ResponseEntity.ok(carResourceAssembler.toResource(car)));
	}

	@PutMapping("/elastic/{brandName}/id/{carid}")
	public String newCarWithElastic(@PathVariable(name = "brandName") String brandname,
			@PathVariable(name = "carid") Integer carid) {

		log.info("Create New Car");
		Car car = carElasticRepository.save(Car.builder().brandName(brandname).build());
		return car.toString();
	}

	@GetMapping("/elasticCustom/{brandName}")
	public @ResponseBody ResponseEntity<?> newCarWithElasticCustom(@PathVariable(name = "brandName") String brandname) {

		log.info("Create New Car");
		// Page<Car> car = carElasticRepository.findByBrandNameCustomQuery(brandname,
		// new PageRequest(1, 10));
		List<Car> cars = findByBrandNameCustomQuery(brandname);
		cars.stream().map(c -> c.getBrandName()).forEach(System.out::println);
		return cars.size() == 0 ? ResponseEntity.notFound().build() : ResponseEntity.ok(cars);
	}

	private List<Car> findByBrandNameCustomQuery(String brandName) {
		
		SearchResponse response = elasticsearchTemplate.getClient().prepareSearch().setTypes()
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH).setPostFilter(QueryBuilders.matchAllQuery())
				// .setPostFilter(QueryBuilders.simpleQueryStringQuery("A"))
				.execute().actionGet();
		List<SearchHit> searchHits = Arrays.asList(response.getHits().getHits());
		List<Car> cars = new ArrayList<Car>();	
		
		searchHits.forEach(hit -> cars.add(JSON.parseObject(hit.getSourceAsString(), Car.class)));
		return cars;
	}*/
